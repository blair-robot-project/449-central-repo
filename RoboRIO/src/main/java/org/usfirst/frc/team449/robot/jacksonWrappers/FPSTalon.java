...
public class FPSTalon implements SimpleMotor, Shiftable, Loggable {
  ...
  @Nullable protected final PDP PDP;
  ...
  /**
   * The minimum number of points that must be in the bottom-level MP buffer before starting a
   * profile.
   */
  private final int minNumPointsInBottomBuffer;
  /** The motion profile motionProfileStatus of the Talon. */
  @NotNull private final MotionProfileStatus motionProfileStatus;
  ...
  /**
   * The period for the {@link Notifier} that updates the feedforward based on the current motion
   * magic velocity setpoint.
   */
   ...
   * @param minNumPointsInBottomBuffer The minimum number of points that must be in the bottom-level
   *     MP buffer before starting a profile. Defaults to 20.
   * @param updaterProcessPeriodSecs The period for the {@link Notifier} that moves points between
   *     the MP buffers, in seconds. Defaults to 0.005.
   * @param updateMMPeriodSecs The period for the {@link Notifier} that updates the feedforward
   *     based on the current motion magic velocity setpoint. Defaults to 0.05.
  ...
  public FPSTalon(
    ...
    this.minNumPointsInBottomBuffer =
    ...
    if (feedbackDevice != null) {
      ...
      //            canTalon.configSelectedFeedbackSensor(FeedbackDevice.None, 0, 0); Uncomment this
      // if FeedbackDevice.None
    ...
    if (slaveVictors != null) {
      ...
      for (SlaveVictor slave : slaveVictors) {
        slave.setMaster(
            canTalon, enableBrakeMode, enableVoltageComp ? notNullVoltageCompSamples : null);
      }
    }
  }
  ...
  public void setPercentVoltage(double percentVoltage) {
    ...
    if (Math.abs(percentVoltage) > 1.0) {
      Logger.addEvent(
          "WARNING: YOU ARE CLIPPING MAX PERCENT VBUS AT " + percentVoltage, this.getClass());
  ...
  public void setGear(int gear) {
    ...
    if (currentGearSettings.motionMagicMaxVel != null) {
      canTalon.configMotionCruiseVelocity(
          FPSToEncoder(currentGearSettings.getMotionMagicMaxVel()).intValue(), 0);
      // We can convert accel the same way we do vel because both are per second.
      canTalon.configMotionAcceleration(
          FPSToEncoder(currentGearSettings.getMotionMagicMaxAccel()).intValue(), 0);
    }
      ...
      if (nativeSetpoint == 0) {
        ...
        canTalon.config_kF(
            ...
            1023.
                / 12.
                / nativeSetpoint
                * currentGearSettings.getFeedForwardComponent().applyAsDouble(feet),
            0);
      }
  ...
  private void updateMotionMagicSetpoint() {
    if (!canTalon.getControlMode().equals(ControlMode.MotionMagic)) {
      ...
      canTalon.set(
          ...
          currentGearSettings
                  .getFeedForwardComponent()
                  .calcMPVoltage(
                      canTalon.getActiveTrajectoryPosition(),
                      canTalon.getActiveTrajectoryVelocity(),
                      0)
              / 12.);
    }
  }
  ...
  /**
   * Get the current velocity setpoint of the Talon in FPS, the position setpoint in feet, or the
   * {@link SetValueMotionProfile} in MP mode.
   *
   ...
   *
   * @return True if minNumPointsInBottomBuffer points have been loaded or the top buffer is empty,
   *     false otherwise.
   */
  public boolean readyForMP() {
    canTalon.getMotionProfileStatus(motionProfileStatus);
    return motionProfileStatus.topBufferCnt == 0
        || motionProfileStatus.btmBufferCnt >= minNumPointsInBottomBuffer;
  }
  ...
  /**
   * Reset all MP-related stuff, including all points loaded in both the API and bottom-level
   * buffers.
   */
  ...
  public void loadProfile(MotionProfileData data) {
    ...
    for (int i = 0; i < data.getData().length; ++i) {
      ...
      // Have to set this so the Talon doesn't throw a null pointer. May be fixed in a future
      // release.
      point.timeDur = 0;
      ...
      point.position =
          feetToEncoder(startPosition + (data.getData()[i][0] * (data.isInverted() ? -1 : 1)));

      if (data.isInverted()) {
        feedforward =
            currentGearSettings
                .getFeedForwardComponent()
                .calcMPVoltage(-data.getData()[i][0], -data.getData()[i][1], -data.getData()[i][2]);
      } else {
        feedforward =
            currentGearSettings
                .getFeedForwardComponent()
      ...
      // Doing vel+accel shouldn't lead to impossible setpoints, so if it does, we log so we know to
      // change
      // either the profile or kA.
      if (Math.abs(feedforward) > 12) {
        System.out.println(
            "Point "
                + Arrays.toString(data.getData()[i])
                + " has an unattainable "
      ...
      point.zeroPos =
          i == 0 && data.resetPosition(); // If it's the first point, set the encoder position to 0.
      point.isLastPoint =
          (i + 1) == data.getData().length; // If it's the last point, isLastPoint = true
   ...
   *
   * @return An N-length array of String labels for data, where N is the length of the Object[]
   *     returned by getData().
   */
  @NotNull
  @Override
  ...
  protected static class PerGearSettings {
    ...
    @Nullable private final Double rampRate;

    /** The maximum speed of the motor in this gear, in FPS. Used for throttle scaling. */
    @Nullable private final Double maxSpeed;
    ...
    /**
     * The component for calculating feedforwards in closed-loop control modes. Ignored if maxSpeed
     * is null.
     */
    @NotNull private final FeedForwardComponent feedForwardComponent;

    /** The maximum velocity for motion magic mode, in FPS. Can be null to not use motion magic. */
    @Nullable private final Double motionMagicMaxVel;
     ...
     *     Defaults to 0.
     * @param motionProfilePFwd The proportional PID constant for forwards motion profiles in this
     *     gear. Ignored if kVFwd is null. Defaults to 0.
     * @param motionProfileIFwd The integral PID constant for forwards motion profiles in this gear.
     *     Ignored if kVFwd is null. Defaults to 0.
     * @param motionProfileDFwd The derivative PID constant for forwards motion profiles in this
     *     gear. Ignored if kVFwd is null. Defaults to 0.
     * @param motionProfilePRev The proportional PID constant for reverse motion profiles in this
     *     gear. Ignored if kVFwd is null. Defaults to motionProfilePFwd.
     * @param motionProfileIRev The integral PID constant for reverse motion profiles in this gear.
     *     Ignored if kVFwd is null. Defaults to motionProfileIFwd.
     * @param motionProfileDRev The derivative PID constant for reverse motion profiles in this
     *     gear. Ignored if kVFwd is null. Defaults to motionProfileDFwd.
     * @param feedForwardComponent The component for calculating feedforwards in closed-loop control
     *     modes. Ignored if maxSpeed is null. Defaults to no feedforward.
     * @param motionMagicMaxVel The maximum velocity for motion magic mode, in FPS. Can be null to
     *     not use motion magic.
    ...
    public PerGearSettings(
      ...
      this.motionProfilePRev =
          motionProfilePRev != null ? motionProfilePRev : this.motionProfilePFwd;
      this.motionProfileIRev =
          motionProfileIRev != null ? motionProfileIRev : this.motionProfileIFwd;
      this.motionProfileDRev =
          motionProfileDRev != null ? motionProfileDRev : this.motionProfileDFwd;
      this.feedForwardComponent =
          feedForwardComponent != null
    ...
    public PerGearSettings() {
      this(
          0, null, null, null, null, null, null, null, 0, 0, 0, 0, 0, 0, null, null, null, null,
          null, 0);
    }
    ...
    /**
     * @return The minimum output voltage for closed-loop modes in the forwards direction. This does
     *     not rescale, it just sets any output below this voltage to this voltage.
     */
    ...
    /**
     * @return The minimum output voltage for closed-loop modes in the reverse direction. This does
     *     not rescale, it just sets any output below this voltage to this voltage.
     */
    ...
    /**
     * @return The maximum velocity for motion magic mode, in FPS. Can be null to not use motion
     *     magic.
     */
    @Nullable
