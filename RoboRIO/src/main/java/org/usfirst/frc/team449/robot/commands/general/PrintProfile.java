...
public class PrintProfile extends InstantCommand {
  ...
  @NotNull private final Supplier<MotionProfileData> left, right;
   ...
   */
  public PrintProfile(
      @NotNull Supplier<MotionProfileData> left, @NotNull Supplier<MotionProfileData> right) {
    this.left = left;
    this.right = right;
  }
  ...
  public void execute() {
    ...
    for (double[] line : leftP.getData()) {
      System.out.println(
          "Position: " + line[0] + ", velocity: " + line[1] + ", acceleration: " + line[2]);
    }
