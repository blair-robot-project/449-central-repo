...
/**
 * A {@link FeedForwardComponent} for use on a drive characterized in the way described in our white
 * paper.
 */
public class FeedForwardKaKvComponent extends FeedForwardComponent {

  /**
   * The voltage required to run forwards or backwards at a steady-state velocity of 1 foot per
   * second.
   */
  private final double kVFwd, kVRev;

  /**
   * The voltage required to accelerate the robot at one foot per second^2 while going forwards or
   * backwards.
   */
   ...
   * @param kARev The voltage required to accelerate the robot at one foot per second^2 while going
   *     in reverse. Defaults to kAFwd.
   * @param interceptVoltageFwd The voltage required to overcome static friction in the forwards
   *     direction. Vintercept in the drive characterization paper. Defaults to 0.
   * @param interceptVoltageRev The voltage required to overcome static friction in the reverse
   *     direction. Vintercept in the drive characterization paper. Defaults to interceptVoltageFwd.
   */
  @JsonCreator
  public FeedForwardKaKvComponent(
      @JsonProperty(required = true) double kVFwd,
    ...
    this.interceptVoltageRev =
        interceptVoltageRev != null ? interceptVoltageRev : this.interceptVoltageFwd;
  }

  /**
   * Calculate the voltage for a setpoint in MP mode with a position, velocity, and acceleration
   * setpoint.
   *
