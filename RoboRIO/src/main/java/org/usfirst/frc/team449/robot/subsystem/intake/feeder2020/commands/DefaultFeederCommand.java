...
public class DefaultFeederCommand extends CommandBase implements Loggable {
  ...
  @NotNull private final ConditionTimingComponentDecorator sensor1, indexing, shooting, flywheelOn;
  @Nullable private final ConditionTimingComponentDecorator sensor2;

  /**
   * Whether or not the feeder has picked up a ball yet. Used in 2021 code, b/c this was the indexer
   * we were using, not the counter
   */
  ...
  public void execute() {
    ...
    if (this.flywheelOn.isTrue()) {
      ...
      // so we must pretend that the sensor has just activated if it was already activated while
      // shooting.
