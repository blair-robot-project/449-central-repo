...
/**
 * Data structure containing the array of points for the MP and a method to fill the MP from a csv
 * file
 */
...
public class MotionProfileData {
  ...
  /**
   * A 2D array containing 4 values for each point- position, velocity, acceleration and delta time
   ...
   * Default constructor
   *
   * @param filename The filename of the .csv with the motion profile data. The first line must be
   *     the number of other lines.
   * @param inverted Whether or not the profile is inverted (would be inverted if we're driving it
   *     backwards)
   * @param velocityOnly Whether or not to only use velocity feed-forward. Used for tuning kV and
   *     kA. Defaults to false.
   * @param resetPosition Whether or not to reset the talon position when this profile starts.
   *     Defaults to false.
   */
  @JsonCreator
  public MotionProfileData(
      @NotNull @JsonProperty(required = true) String filename,
   ...
   */
  public MotionProfileData(
      @NotNull List<Double> pos,
      @NotNull List<Double> vel,
      @NotNull List<Double> accel,
      double deltaTime,
      boolean inverted,
      boolean velocityOnly,
      boolean resetPosition) {
  ...
  /**
   * @return A 2D array containing 3 values for each point- position, velocity, and delta time
   *     respectively.
   */
  @NotNull
