...
public class PathGenerator {
  ...
  @NotNull private final MotionProfileConstraints fwdSettings, revSettings;
  ...
  /**
   * The current point being loaded from 254's profile into a {@link MotionProfileData}. Field to
   * avoid garbage collection.
   */
  private MotionState motionState;

  /**
   * The list of positions, velocities, and accelerations in the profile. Field to avoid garbage
   * collection.
   */
   ...
   * @param revMaxVel The maximum speed (not velocity, this should be positive) for the profile to
   *     reach going in reverse, in feet/sec. Defaults to fwdMaxVel.
   * @param maxAccel The maximum acceleration for the profile to reach, in feet/(sec^2)
   * @param deltaTimeSecs The amount of time between each point, in seconds. Defaults to .02
   *     seconds, or 20 milliseconds.
   */
  @JsonCreator
  public PathGenerator(
      @JsonProperty(required = true) double fwdMaxVel,
    ...
    this.revSettings =
        new MotionProfileConstraints(revMaxVel != null ? revMaxVel : fwdMaxVel, maxAccel);
   ...
   */
  public MotionProfileData generateProfile(
      double currentPos, double currentVel, double currentAccel, double destinationPos) {
    ...
    vel.add(motionState.vel());
    accel.add(motionState.acc());
    return new MotionProfileData(
        pos, vel, accel, deltaTimeSecs, currentPos > destinationPos, false, false);
  }
}
