...
public class UnidirectionalPoseEstimator<T extends SubsystemAHRS & DriveUnidirectional>
    implements PoseEstimator, Loggable {
  /** The subsystem to get gyro and encoder data from. */
  @NotNull private final T subsystem;

  /**
   * The maximum amount, in degrees, a new absolute position's angle can be off from the gyro
   * reading and still be accepted as valid.
   */
  ...
  @NotNull private List<Double> angles;

  /**
   * A list of all [x,y] transformation vectors calculated, in order from oldest to newest and in
   * feet.
   */
  @NotNull private List<double[]> vectors;

  /**
   * A list of all the times at which the gyro angles and vectors were recorded, in order from
   * oldest to newest and in milliseconds.
   */
  @NotNull private List<Long> times;

  /** The current x,y position of the robot, in feet. */
  private double[] currentPos;

  /**
   * The time, in milliseconds since the robot code started, that the last absolute position was
   * recorded at.
   */
  ...
  /**
   * The angle of the gyro the last time the loop ran, in degrees. This is normally the same as the
   * last element of angles, but it's possible that a new absolute position could erase all elements
   * of angles.
   */
  private double lastTheta;

  /**
   * The last time the loop ran, in milliseconds. This is normally the same as the last element of
   * times, but it's possible that a new absolute position could erase all elements of times.
   */
   ...
   * @param absolutePosAngleTolerance The maximum amount, in degrees, a new absolute position's
   *     angle can be off from the gyro reading and still be accepted as valid.
   ...
   */
  @JsonCreator
  public UnidirectionalPoseEstimator(
  ...
  private double[] calcVector(double left, double right, double deltaTheta, double lastAngle) {
    ...
    if (deltaTheta == 0) {
      return new double[] {
        (left + right) / 2. * Math.cos(lastAngle), (left + right) / 2. * Math.sin(lastAngle)
      };
    } else {
      ...
      return new double[] {
        vectorMagnitude * Math.cos(vectorAngle), vectorMagnitude * Math.sin(vectorAngle)
      };
    }
  }

  /**
   * Use the current gyro and encoder data to calculate how the robot has moved since the last time
   * run was called.
   */
  @Override
  public synchronized void run() {
    // Record everything at the start, as it may change between executing lines of code and that
    // would be bad.
    ...
    // If we received an absolute position between the last run and this one, scale the vector so it
    // only includes
   ...
   * @param time The time, in milleseconds after the robot code started, that the absolute position
   *     was recorded.
   * @return true if the absolute position was the most recent received and was used, false
   *     otherwise.
   */
  ...
  /**
   * Add an absolute position at the given time stamp, using an angle measured to verify that the
   * absolute position is correct.
   *
   * @param x The absolute x, in feet
   * @param y The absolute y, in feet
   * @param time The time, in milleseconds after the robot code started, that the absolute position
   ...
   * @return true if the absolute position was the most recent received and the angle was correct
   *     enough to be used, false otherwise.
   */
  public synchronized boolean addAbsolutePos(double x, double y, long time, double angle) {
    ...
    if (firstKeepableIndex == 0) {
      // If the absolute position is from before the first angle was recorded, just use the first
      // angle.
      angleAtTime = angles.get(0);
    } else {
      // Calculate the angle at the time by assuming constant angular velocity over the interval.
      angleAtTime =
          (angles.get(firstKeepableIndex - 1) * (time - times.get(firstKeepableIndex - 1))
    ...
    // If the angle from the gyro and from the absolute position are too different, don't use the
    // absolute position.
   ...
   * @param time The time, in milleseconds after the robot code started, that the absolute position
   *     was recorded
   * @param firstKeepableIndex The first index of the times, vectors, and angles arrays recorded
  ...
  private void addPos(double x, double y, long time, int firstKeepableIndex) {
    ...
    // The "first keepable" vector starts before the absolute position is measured and ends after
    // it, so we have to
    // add the part that comes after to the position.
    if (firstKeepableIndex
        > 0) { // This if is just to prevent IndexOutOfBounds on times.get(firstKeepableIndex - 1)
      this.currentPos[0] +=
          vectors.get(firstKeepableIndex)[0]
              * (times.get(firstKeepableIndex) - time)
              / (times.get(firstKeepableIndex) - times.get(firstKeepableIndex - 1));
      this.currentPos[1] +=
          vectors.get(firstKeepableIndex)[1]
              * (times.get(firstKeepableIndex) - time)
              / (times.get(firstKeepableIndex) - times.get(firstKeepableIndex - 1));
    }
    ...
    // Add all the vectors that come after the absolutePos to the position. Loop starts at index 1
    // because index 0,
  ...
  public String[] getHeader() {
    return new String[] {"effective_wheelbase", "x_displacement", "y_displacement"};
  }
  ...
  public Object[] getData() {
    return new Object[] {fudgedWheelbaseDiameter, getPos()[0], getPos()[1]};
  }
