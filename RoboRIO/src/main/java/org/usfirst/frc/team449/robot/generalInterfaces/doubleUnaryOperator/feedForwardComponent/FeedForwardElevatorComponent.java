...
public class FeedForwardElevatorComponent extends FeedForwardComponent {
  ...
  @NotNull private final double[] voltages;
   ...
   * Default constructor.
   *
   * @param feetToVoltageMap A map of the upper heights of each elevator stage to the voltage
   *     required to counter gravity at that stage.
   */
  @JsonCreator
  public FeedForwardElevatorComponent(
      @NotNull @JsonProperty(required = true) Map<Double, Double> feetToVoltageMap) {
  ...
  public double applyAsDouble(double operand) {
    ...
    for (int i = 0; i < positions.length; i++) {
      if (pos <= positions[i]) {
        // 1023/12 converts from voltage to native, and divide by operand because FF gets multiplied
        // by setpoint
