...
/** TODO test the changes A command that runs a command from a dictionary. */
public abstract class SelectCommand<T> extends CommandBase {
  ...
  /**
   * Creates a new SelectCommand with given map of selectors and m_commands. <br>
  ...
  private void requireAll() {
    for (Command c : m_commands.values())
      for (Subsystem requirement : c.getRequirements()) addRequirements(requirement);
  }
