...
public class PathRequester {
  ...
  @NotNull private final ZMQ.Socket socket;

  /** The path received from the Jetson. Field to avoid garbage collection. */
  private PathOuterClass.Path path;

  /**
   * The proto for requesting a motion profile from the JetsopathRequestern. Field to avoid garbage
   * collection.
   */
  ...
  public void requestPath(
      Waypoint[] waypoints, double deltaTime, double maxVel, double maxAccel, double maxJerk) {
    // Send the request
   ...
   * @return Null if the Jetson hasn't replied yet, a list of one profile if theta is 0, or a list
   *     of left, right profiles in that order otherwise.
   */
  @Nullable
  public MotionProfileData[] getPath(boolean inverted, boolean resetPosition) {
    ...
    // Make these local variables and not fields so that this thread doesn't retain any connection
    // to it.
    ...
    try {
      ...
      leftMotionProfileData =
          new MotionProfileData(
              path.getPosLeftList(),
              path.getVelLeftList(),
              path.getAccelLeftList(),
              path.getDeltaTime(),
              inverted,
              false,
              resetPosition);
      if (path.getPosRightCount() != 0) {
        rightMotionProfileData =
            new MotionProfileData(
                path.getPosRightList(),
                path.getVelRightList(),
                path.getAccelRightList(),
                path.getDeltaTime(),
                inverted,
                false,
                resetPosition);
