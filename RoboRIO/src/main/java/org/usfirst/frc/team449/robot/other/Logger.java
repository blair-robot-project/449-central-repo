...
/**
 * A logger that logs telemetry data and individual events. Should be run as a separate thread from
 * the main robot loop.
 */
...
public class Logger implements Runnable {
  ...
  @NotNull private static List<LogEvent> events = new ArrayList<>();
  ...
  /**
   * The time this that logging started. We don't use {@link Clock} because this is a separate
   * thread.
   */
  private final long startTime;

  /** The filewriters for writing to the telemetry and event logs. */
  @NotNull private final FileWriter telemetryLogWriter, eventLogWriter;
   ...
   * @param eventLogFilename The filepath of the log for events. Will have the timestamp and file
   *     extension appended onto the end.
   * @param telemetryLogFilename The filepath of the log for telemetry data. Will have the timestamp
   *     and file extension appended onto the end.
  ...
  /**
   * Add a loggable to be logged. This must be called before a Logger is constructed, and so should
   * be called in the constructor of a Loggable.
   *
  ...
  /**
   * Print out all logged events to the event log and write all the telemetry data to the telemetry
   * log.
   */
  @Override
  public void run() {
    ...
    for (int i = 0; i < loggables.length; i++) {
      ...
      for (int j = 0; j < data.length; j++) {
        ...
        // We do this big thing here so we log it to SmartDashboard as the correct data type, so we
        // make each
    ...
    try {
      telemetryLogWriter.write(
          telemetryData.toString().substring(0, telemetryData.length() - 1) + "\n");
      telemetryLogWriter.flush();
    } catch (Exception e) {
      System.out.println("Logging failed!");
      e.printStackTrace();
