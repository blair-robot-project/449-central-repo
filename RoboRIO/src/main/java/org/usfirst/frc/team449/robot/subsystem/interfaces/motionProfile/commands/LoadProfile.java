...
public class LoadProfile<T extends Subsystem & SubsystemMP> extends InstantCommand {
  ...
  @Nullable private final MotionProfileData profile;

  /**
   * A Supplier that gets the profile to load. Ignored if profile isn't null, must be non-null if
   * profile is null.
   */
  @Nullable private final Supplier<MotionProfileData> profileSupplier;
   ...
   */
  public LoadProfile(
      @NotNull SubsystemMP subsystem, @NotNull Supplier<MotionProfileData> profileSupplier) {
    this.subsystem = subsystem;
