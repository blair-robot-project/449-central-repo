...
/**
 * A {@link FeedForwardComponent} for an elevator, where the feed-forward needed changes based on
 * how many stages are lifted.
 */
public class FeedForwardMPElevatorComponent extends FeedForwardComponent {
  ...
  @NotNull private final Double[] positions;

  /** The feed forwards at each elevator stage, in ascending order. */
  @NotNull private final FeedForwardComponent[] feedForwardComponents;
   ...
   * Default constructor.
   *
   * @param feetToFFComponentMap A map of the upper heights of each elevator stage to the {@link
   *     FeedForwardComponent} that should be used for that stage.
   */
  @JsonCreator
  public FeedForwardMPElevatorComponent(
      @NotNull @JsonProperty(required = true)
          Map<Double, FeedForwardComponent> feetToFFComponentMap) {
  ...
  public double calcMPVoltage(double positionSetpoint, double velSetpoint, double accelSetpoint) {
    ...
    for (int i = 0; i < positions.length; i++) {
      if (positionSetpoint <= positions[i]) {
        return (feedForwardComponents[i].calcMPVoltage(
            positionSetpoint, velSetpoint, accelSetpoint));
      }
    }
