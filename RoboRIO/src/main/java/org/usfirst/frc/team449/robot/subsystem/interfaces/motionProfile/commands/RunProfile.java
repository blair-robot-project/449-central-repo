...
public class RunProfile<T extends Subsystem & SubsystemMPTwoSides> extends SequentialCommandGroup {
  ...
  public RunProfile(
    ...
    addCommands(new LoadProfile<>(subsystem, profile), new RunLoadedProfile<>(subsystem, timeout));
  }
   ...
   */
  public RunProfile(
      @NotNull T subsystem, @NotNull Supplier<MotionProfileData> profileSupplier, double timeout) {
    addCommands(
        new LoadProfile<>(subsystem, profileSupplier), new RunLoadedProfile<>(subsystem, timeout));
  }
}
